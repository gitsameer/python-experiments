...
def getGroupMembersDict(group):
  ...
  query.group = groupIDDict[group]
   

  feed = gd_client.GetContactsFeed(query.ToUri())

  websiteContactIdDict= {}
   
  for contact in feed.entry:
        csid = contact.website[0].href.split("/")[-1]
   ...
   #print rel_list[phone.rel] + ": " + phone.text
        websiteContactIdDict[csid] = contact.id.text
  
  return websiteContactIdDict 

dict = getGroupMembersDict('Murmansk')
print dict 
